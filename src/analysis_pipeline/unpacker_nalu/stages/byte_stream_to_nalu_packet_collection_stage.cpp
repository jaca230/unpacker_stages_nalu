...
void ByteStreamToNaluPacketCollectionStage::Process() {
    ...
    // Step 3: Parse N packets
    auto collection = std::make_unique<dataProducts::NaluPacketCollection>();

    for (int i = 0; i < repeat_count; ++i) {
        if (offset + header_parser_->GetTotalParsedSize()
                  + payload_parser_->GetTotalParsedSize()
                  + footer_parser_->GetTotalParsedSize() > buffer_size) {
            spdlog::warn("[{}] Buffer too small for {}th packet", Name(), i);
            break;
        }

        auto header = std::make_unique<dataProducts::NaluPacketHeader>();
        if (!header_parser_->Parse(buffer, buffer_size, offset, header.get())) break;
        offset += header_parser_->GetTotalParsedSize();

        auto payload = std::make_unique<dataProducts::NaluPacketPayload>();
        if (!payload_parser_->Parse(buffer, buffer_size, offset, payload.get())) break;
        offset += payload_parser_->GetTotalParsedSize();

        auto footer = std::make_unique<dataProducts::NaluPacketFooter>();
        if (!footer_parser_->Parse(buffer, buffer_size, offset, footer.get())) break;
        offset += footer_parser_->GetTotalParsedSize();

        dataProducts::NaluPacket pkt;
        pkt.header = std::move(*header);
        pkt.payload = std::move(*payload);
        pkt.footer = std::move(*footer);
